...
for i in range(len(files)):
    ...
    if os.path.isfile(filepath):
        # print('********    file   ********', file)
        image1 = cv2.imread(folder + '/enhancement/' + file1)
        # h, w, _ = image1.shape
        # h, w = h // 2, w // 2
        # x = int(h / 2 - 512 / 2)
        # y = int(w / 2 - 512 / 2)
        # # print(x, y)
        # image1 = image1[2 * x:2 * x + 1024, 2 * y:2 * y + 1024, :]
        # cv2.imwrite(folder + '\\pro\\' + file1,image1)
        image = cv2.imread(folder + '/GT/' + file)

        # image=  image[2 * x:2 * x + 1024, 2 * y:2 * y + 1024, :]
        # cv2.imwrite(folder + '\\gt\\' + file, image)
