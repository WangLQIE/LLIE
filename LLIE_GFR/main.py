...
from GF_FCA_Lewin import EnhanceNet
...
parser.add_argument('--epoch', dest='epoch', type=int, default=4000, help='number of total epoches')
...
parser.add_argument('--dir', dest='dir', default='ablation_lewin_v2', help='directory')
parser.add_argument('--test_dir', dest='test_dir', help='directory for testing inputs')# F:/chenlong/Dataset_Sony/Sony/short E:/WXJ/ori_raw1/Sony/short
parser.add_argument('--train_dir', dest='train_dir', help='directory for training inputs')# 路径后面有斜杆
parser.add_argument('--continue_train',dest='continue_train',default=0)

args = parser.parse_args()
os.environ["CUDA_VISIBLE_DEVICES6"] = args.gpu_idx
...
def train():
    ...
    if args.continue_train:
        checkpoint = torch.load(r"state_final.pth")
    ...
    w_log.close()
def test():
    checkpoint = torch.load("checkpoint_"+args.dir+'/4000_state_final.pth')
    ...
    with torch.no_grad():
        for (step,data) in enumerate(dataLoader):
            ...
            save_images(os.path.join('test_results_'+args.dir,'%d_00_%d_out.png'%(idstr,ratio)),out_cpu.detach().numpy())
            save_images(os.path.join('test_results_'+args.dir,'%d_00_%d_gt.png'%(idstr,ratio)),high_im.detach().numpy())
            torch.cuda.empty_cache()
def main():
    if args.use_gpu:
        if args.phase == 'train':
